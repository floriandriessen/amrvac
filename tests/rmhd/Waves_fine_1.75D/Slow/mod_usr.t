...
contains
  ...
  subroutine initglobaldata_usr
    ...
    if (B0x .eq. 0) then
      ...
      A_vy = (B0y/B0x)*(omega/wavenumber)*(ampl/rho0)/(1.0d0 - (omega**2)/((vax**2)*(wavenumber**2)))
...
113    close(unitpar)
  ...
  subroutine initial_conditions(ixI^L, ixO^L, w, x)
    ...
    *(unit_pressure/unit_density)/unit_temperature
    ...
    w(ixI^S, r_e) = Er0!const_rad_a*(temp(ixI^S)*unit_temperature)**4.d0/unit_pressure
  ...
  subroutine Initialize_Wave(level,qt,ixI^L,ixO^L,w,x)
    ...
    where (x(ixI^S,1) .lt. one)
      ...
      *(unit_pressure/unit_density)/unit_temperature
      ...
      w(ixI^S, r_e) = Er0 + A_Er*dsin(wavenumber*x(ixI^S,1)-omega*global_time)
