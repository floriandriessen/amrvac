...
contains
  ...
  subroutine initglobaldata_usr
    ...
    cslow = dsqrt(0.5d0*((va*va + ca*ca) - dsqrt((va*va + ca*ca)*(va*va + &
       ca*ca) - 4.0d0*ca*ca*vax*vax)))
    cfast = dsqrt(0.5d0*((va*va + ca*ca) + dsqrt((va*va + ca*ca)*(va*va + &
       ca*ca) - 4.0d0*ca*ca*vax*vax)))
    ...
    if (mype .eq. 0) then
      ...
      print*, '----------------------------- after non-dimensionalization'
      print*, 'rho0', rho0
      print*, 'p0', p0
      print*, 'T0', T0
      print*, 'a0', a0
    ...
    A_p = (ca**2)*A_rho
    A_By = B0y*(ampl/rho0)/(1.0d0 - (vax**2)*(wavenumber**2)/(omega**2))
    A_Bz = B0z*(ampl/rho0)/(1.0d0 - (vax**2)*(wavenumber**2)/(omega**2))
    if (B0x .eq. 0) then
      A_vy = 0.d0
      A_vz = 0.d0
    else
      A_vy = (B0y/B0x)*(omega/wavenumber)*(ampl/rho0)/(1.0d0 - &
         (omega**2)/((vax**2)*(wavenumber**2)))
      A_vz = (B0z/B0x)*(omega/wavenumber)*(ampl/rho0)/(1.0d0 - &
         (omega**2)/((vax**2)*(wavenumber**2)))
    endif
...
113    close(unitpar)
  ...
  subroutine initial_conditions(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x)
    ...
    double precision :: press(ixImin1:ixImax1), temp(ixImin1:ixImax1)
    ...
    w(ixImin1:ixImax1, e_) = eg0

    press(ixImin1:ixImax1) = p0
    temp(ixImin1:ixImax1) = (const_mp*fld_mu/const_kb)*press(&
       ixImin1:ixImax1)/w(ixImin1:ixImax1,&
        rho_)*(unit_pressure/unit_density)/unit_temperature

    w(ixImin1:ixImax1, r_e) = Er0 !const_rad_a*(temp(ixImin1:ixImax1)*unit_temperature)**4.d0/unit_pressure

    if(stagger_grid) then
      call b_from_vector_potential(block%ixGsmin1,block%ixGsmax1,ixImin1,&
         ixImax1,ixOmin1,ixOmax1,block%ws,x)
      call rmhd_face_to_center(ixOmin1,ixOmax1,block)
    else
      w(ixImin1:ixImax1,mag(:))= 0.d0
      w(ixImin1:ixImax1,mag(1))= B0x
      w(ixImin1:ixImax1,mag(2))= B0y
      w(ixImin1:ixImax1,mag(3))= B0z
    end if
  ...
  subroutine Initialize_Wave(level,qt,qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,w,x)
    ...
    where (x(ixImin1:ixImax1,1) .lt. one)
      w(ixImin1:ixImax1, rho_) = rho0 + A_rho*dsin(wavenumber*x(&
         ixImin1:ixImax1,1)-omega*global_time)
      
      
      w(ixImin1:ixImax1, mom(1)) = w(ixImin1:ixImax1,&
          rho_)*A_v*dsin(wavenumber*x(ixImin1:ixImax1,1)-omega*global_time)
      w(ixImin1:ixImax1, e_) = eg0 + A_e*dsin(wavenumber*x(ixImin1:ixImax1,&
         1)-omega*global_time)

      press(ixImin1:ixImax1) = p0 + A_p*dsin(wavenumber*x(ixImin1:ixImax1,&
         1)-omega*global_time)
      temp(ixImin1:ixImax1) = (const_mp*fld_mu/const_kb)*press(&
         ixImin1:ixImax1)/w(ixImin1:ixImax1,&
          rho_)*(unit_pressure/unit_density)/unit_temperature
      ...
      w(ixImin1:ixImax1, r_e) = Er0 + A_Er*dsin(wavenumber*x(ixImin1:ixImax1,&
         1)-omega*global_time)

      w(ixImin1:ixImax1, mag(1)) = B0x
      w(ixImin1:ixImax1, mag(2)) = B0y + &
         A_By*dsin(wavenumber*x(ixImin1:ixImax1,1)-omega*global_time)
      w(ixImin1:ixImax1, mag(3)) = B0z + &
         A_Bz*dsin(wavenumber*x(ixImin1:ixImax1,1)-omega*global_time)

      w(ixImin1:ixImax1, mom(2)) = w(ixImin1:ixImax1,&
          rho_)*A_vy*dsin(wavenumber*x(ixImin1:ixImax1,1)-omega*global_time)
      w(ixImin1:ixImax1, mom(3)) = w(ixImin1:ixImax1,&
          rho_)*A_vz*dsin(wavenumber*x(ixImin1:ixImax1,1)-omega*global_time)
  ...
  subroutine boundary_conditions(qt,ixImin1,ixImax1,ixBmin1,ixBmax1,iB,w,x)
    ...
    case(2)
      ...
      call eos%to_primitive(ixImin1,ixImax1,ixBmin1,ixBmax1,w,x)
      ...
      call eos%to_conserved(ixImin1,ixImax1,ixBmin1,ixBmax1,w,x)
    case default
