...
contains
  ...
  subroutine initglobaldata_usr
    ...
    eg1 = p1/(eos%gamma-1.d0) + half*rho1*(v1*v1 + vy1*vy1 + vz1*vz1) + &
       half*(B1*B1 + By1*By1 + Bz1*Bz1)/(4.0*dpi)
    eg2 = p2/(eos%gamma-1.d0) + half*rho2*(v2*v2 + vy2*vy2 + vz2*vz2) + &
       half*(B2*B2 + By2*By2 + Bz2*Bz2)/(4.0*dpi) 
    ...
    ((p2+eg2+4.0*Er2/3.0 + half*(B2*B2+By2*By2+Bz2*Bz2))*v2 - &
       B2*(v2*B2+vy2*By2+vz2*Bz2))
  ...
  subroutine params_read(files)
    ...
    namelist /shock_list/ rho1, rho2, v1, v2, T1, T2, B1, B2, vy1, vz1, By1,&
        Bz1, vy2, vz2, By2, Bz2
...
113    close(unitpar)
  ...
  subroutine initial_conditions(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x)
    ...
    double precision :: rho(ixImin1:ixImax1), Temp(ixImin1:ixImax1),&
        press(ixImin1:ixImax1), vel(ixImin1:ixImax1)
    ...
    w(ixImin1:ixImax1,mom(2)) = rho1*vy1 
    w(ixImin1:ixImax1,mom(3)) = rho1*vz1 
    w(ixImin1:ixImax1,e_) = eg1
    w(ixImin1:ixImax1,r_e) = Er1

    w(ixImin1:ixImax1,mag(:)) = 0.d0
    w(ixImin1:ixImax1,mag(1)) = B1
    w(ixImin1:ixImax1,mag(2)) = By1 
    w(ixImin1:ixImax1,mag(3)) = Bz1 

    where (x(ixImin1:ixImax1,1) .gt. 0.d0)
      w(ixImin1:ixImax1,rho_) = rho2
      w(ixImin1:ixImax1,mom(1)) = rho2*v2
      w(ixImin1:ixImax1,mom(2)) = rho2*vy2 
      w(ixImin1:ixImax1,mom(3)) = rho2*vz2 
      w(ixImin1:ixImax1,e_) = eg2
      w(ixImin1:ixImax1,r_e) = Er2
      w(ixImin1:ixImax1,mag(1)) = B2
      w(ixImin1:ixImax1,mag(2)) = By2 
      w(ixImin1:ixImax1,mag(3)) = Bz2 
    end where
  ...
  subroutine boundary_conditions(qdt,qt,ixImin1,ixImax1,ixBmin1,ixBmax1,iB,w,&
    ...
    case(1)
      ...
      w(ixBmin1:ixBmax1,mom(2)) = rho1*vy1
      w(ixBmin1:ixBmax1,mom(3)) = rho1*vz1
      w(ixBmin1:ixBmax1,e_) = eg1
      w(ixBmin1:ixBmax1,r_e) = Er1
      w(ixBmin1:ixBmax1,mag(:)) = 0.0d0
      w(ixBmin1:ixBmax1,mag(1)) = B1
      w(ixBmin1:ixBmax1,mag(2)) = By1
      w(ixBmin1:ixBmax1,mag(3)) = Bz1

    case(2)
      w(ixBmin1:ixBmax1,rho_) = rho2
      w(ixBmin1:ixBmax1,mom(:)) = 0.d0
      w(ixBmin1:ixBmax1,mom(1)) = rho2*v2  
      w(ixBmin1:ixBmax1,mom(2)) = rho2*vy2 
      w(ixBmin1:ixBmax1,mom(3)) = rho2*vz2 
      w(ixBmin1:ixBmax1,e_) = eg2
      w(ixBmin1:ixBmax1,r_e) = Er2
      w(ixBmin1:ixBmax1,mag(:)) = 0.0d0
      w(ixBmin1:ixBmax1,mag(1)) = B2
      w(ixBmin1:ixBmax1,mag(2)) = By2
      w(ixBmin1:ixBmax1,mag(3)) = Bz2
  ...
  subroutine specialvar_output(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x,normconv)
    ...
    double precision :: lamb(ixOmin1:ixOmax1), R(ixOmin1:ixOmax1)

    call fld_get_fluxlimiter(w,x,ixImin1,ixImax1,ixOmin1,ixOmax1,lamb,R,1)
