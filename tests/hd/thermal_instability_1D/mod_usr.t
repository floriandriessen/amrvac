!=============================================================================
! 1D HD setup for thermal instability
...
module mod_usr
  ...
  integer :: temp_, vel_, press_, thcond_, rad_, heat_, netheat_, amr_
...
111    close(unitpar)
   ...
   subroutine initonegrid_usr(ixG^L,ix^L,w,x)    
    ...
    k = 2.d0 * dpi / (xprobmax1 - xprobmin1) ! k for the fundamental mode
    
    ! 1. Density: Perturbed in a cosine wave
    factorrho(ix^S)=1.0d0-A0*dcos(k*x(ix^S,1))
    ...
    if(mype==0.and.first)then
        ...
        write(*,*)'k is=',k
        write(*,*)'domain=',xprobmin1,xprobmax1
        first=.false.
    endif
  ...
  subroutine set_output_vars(ixI^L,ixO^L,qt,w,x)
    ...
    w(ixO^S,vel_)=wlocal(ixO^S,mom(1))
