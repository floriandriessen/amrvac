...
program amrvac
  ...
  use mod_physics
  use mod_eos, only: eos_init, eos_finalise, prepare_eos_w_fields
  ...
  call initialize_amrvac() !> Grid bounds are set up here (ixG1_,ixG2_,ixG3_ etc)
  ...
  if (mype==0) then
     ...
     print*,'-------------------------------------------------------------------------------'
     write(*,'(a,f17.3,a,f17.3,a)')' Finished AMRVAC in : ',time0,' sec',&
         dble(npe)*time0/3.6d3,' core hour'
     print*,'-------------------------------------------------------------------------------'
  end if
...
contains
  ...
  subroutine timeintegration()
    ...
    if (mype==0) then
      ...
      write(*, '(A4,A10,A12,A12,A12,A14)') '  #', 'it', 'time', 'dt',&
          'wc-time(s)', 'active_grids'
    end if
    ...
    time_evol : do
       ...
       if (timeio0 - time_last_print > time_between_print) then
         ...
         if (mype == 0) then
           write(*, '(A4,I10,ES12.4,ES12.4,ES12.4,I14)') " #", it, global_time,&
               dt, timeio0 - time_in, nleafs_active
         end if
       end if
       ...
       timegr0=MPI_WTIME()
       if (mod(it,ditregrid)==0 .and. refine_max_level>1 .and. &
          .not.(fixgrid())) call resettree
       ...
       if (mype==0 .and. timing_log_opened .and. mod(it,&
           timing_log_interval)==0) then
         write(timing_unit,'(a,i8,1x,es12.5,1x,i6,1x,9(f10.4,1x))') '  MAIN ',&
             it, global_time, nleafs_active, tw_setdt, tw_advance, tw_tc_total,&
             tw_eos_hydro, tw_eos_tc, tw_regrid, tw_save, tw_process,&
             tw_advance - tw_tc_total
    ...
    if (mype==0) then
       ...
       timeeos_tot = timeeos_update + timeeos_Tfromei + timeeos_csound + &
          timeeos_conv + timeeos_pthermal
       write(*,'(a,f12.3,a)')' Time spent on eos          : ',timeeos_tot,&
          ' sec'
       write(*,'(a,f12.2,a)')'                  Percentage: ',&
          100.0*timeeos_tot/timeloop,' %'
       write(*,'(a,f12.3,a)')'   - update_eos             : ',timeeos_update,&
          ' sec'
       write(*,'(a,f12.3,a)')'   - T_from_eint (TC)       : ',timeeos_Tfromei,&
          ' sec'
       write(*,'(a,f12.3,a)')'   - csound2                : ',timeeos_csound,&
          ' sec'
       write(*,'(a,f12.3,a)')'   - cons/prim conversion   : ',timeeos_conv,&
          ' sec'
       write(*,'(a,f12.3,a)')'   - get_pthermal           : ',timeeos_pthermal,&
          ' sec'
       write(*,'(a,f12.3,a)')' Time spent on WB transform : ',&
          time_wb_transform,' sec'
       write(*,'(a,f12.3,a)')' Time spent on WB inverse+C : ',time_wb_inverse,&
          ' sec'
       write(*,'(a,f12.3,a)')' Time spent on reconstruction:',time_wb_recon,&
          ' sec'
       write(*,'(a,f12.3,a)')' Time spent on computing    : ',&
          timeloop-timeio_tot-timeeos_tot-timegr_tot-time_bc,' sec'
       write(*,'(a,f12.2,a)')'                  Percentage: ',&
          100.0*(timeloop-timeio_tot-timeeos_tot-timegr_tot-time_bc)/timeloop,&
          ' %'
