module mod_usr
  ...
  double precision :: periods=300.d0, variation=75.d0, si=5./6. 
  integer :: vtim=100, vlim=768, num = 1000, mnx = 768
contains
  ...
  subroutine initglobaldata_usr()
    heatunit=unit_pressure/unit_time          ! 3.697693390805347E-003 erg*cm^-3*s-1
    trelax=0.d0                              ! relax time
    ...
    call inithdstatic
    call generateTV(vtim,vlim,periods,variation,mnx,num,si)
  ...
  subroutine getlQ(lQgrid,ixI^L,ixO^L,qt,w,x)
    ...
    mode=1
    call getlQgrid(lQgrid,ixI^L,ixO^L,qt,trelax,x,mode,vtim,vlim)

    call initvecpot_usr(ixI^L, ixO^L, x, A, 3)
    Atop=B0/kx*cos(kx*0.d0)
    Alow=B0/kx*cos(kx*3.0d0)
  ...
  ! corresponding normalization values (default value 1)
    ...
    logical :: lrlt(ixI^S)
    double precision :: flux(ixI^S),fluxx(ixI^S),em(ixI^S),en(ixI^S)
    ...
    ! output background heating rate
    call getbQ(ens,ixI^L,ixO^L,global_time,wlocal,x)
    w(ixO^S,nw+5)=ens(ixO^S)

    ! output localized heating rate
    call getlQ(ens,ixI^L,ixO^L,global_time,wlocal,x)
  ...
  ! newly added variables need to be concatenated with the w_names/primnames string
    ...
    varnames='Te Alfv divB beta bQ lQ j1 j2 j3'
