module mod_usr
  ...
  double precision, parameter :: cos_modes(ndim)      = [0.5d0, 1.0d0]
  double precision, parameter :: gauss_r0(ndim)       = [0.0d0, 0.0d0]
...
contains
  ...
  subroutine initial_conditions(ixG^L, ix^L, w,x)
    ...
    ! Analytic solution
    w(ix^S, rho_) = solution(x(ix^S, 1), x(ix^S, 2))

    ! Analytic right-hand side
    w(ix^S, i_rhs) = rhs(x(ix^S, 1), x(ix^S, 2))
  ...
  subroutine my_refine(igrid,level,ixI^L,ixO^L,qt,w,x,refine,coarsen)
    ...
    case ('corner')
       if (any(abs(x(ixO^S, 1) + 0.25d0) <= 0.1d0) .and. &
            any(abs(x(ixO^S, 2) + 0.25d0) <= 0.1d0)) then
          refine = 1
          coarsen = -1
       else
          refine = -1
