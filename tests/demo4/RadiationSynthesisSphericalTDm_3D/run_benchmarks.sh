...
usage() {
  cat <<'EOF'
Usage:
  tests/demo4/RadiationSynthesisSphericalTDm_3D/run_benchmarks.sh
...
case_names=(u128 u256 u512 amr_l3 amr_l4 amr_l3_rstretch)
...
for case_name in "${case_names[@]}"; do
  ...
  snapshot="output/radsyn_sph_prom_${case_name}_0000.dat"
  ...
  for np in ${core_list}; do
    ...
    printf '%s,%s,%s,%s,%s,%s\n' \
      "spherical" "${case_name}" "${np}" "${elapsed}" "${bench_par}" "${log_file}" \
      >> output/benchmark/radsyn_benchmark_times.csv
  done
done
