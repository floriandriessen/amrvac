...
contains
  ...
  subroutine usr_init()
    ...
    !> artificial fixing velocities
    usr_internal_bc => floor_velocity
    ...
    if (hd_energy) i_p = var_set_extravar("p","p")
    i_Trad = var_set_extravar("Trad", "Trad")
    if (hd_energy) i_Tgas = var_set_extravar("Tgas", "Tgas")
...
113    close(unitpar)
  ...
  subroutine initial_conditions(ixI^L, ixO^L, w, x)
    ...
    do ii = ixOmin1,ixOmax1
      ...
      w(ii,:,e_) = read_initial_conditions(x(ii,3,1),4)/unit_pressure
...
8765 CLOSE(1)
    ...
    if ((var .lt. 1.d-12*unit_density) .and. (index .eq. 2)) then
      call mpistop("some issue with reading 1D_stable")
    end if
  ...
  subroutine boundary_conditions(qdt,qt,ixI^L,ixB^L,iB,w,x)
    ...
    case(1)
      ...
      do ix1 = ixBmax1,ixBmin1,-1
        w(ix1,ixBmin2:ixBmax2,mom(1)) = w(ix1+1,ixBmin2:ixBmax2,mom(1))*x(ix1+1,ixBmin2:ixBmax2,1)**2/x(ix1,ixBmin2:ixBmax2,1)**2
      ...
      w(ixB^S,e_) = w(ixB^S,rho_)*temp(ixB^S)/(eos%gamma-1.d0) + half*(w(ixB^S,mom(1))**2+w(ixB^S,mom(2))**2)/w(ixB^S,rho_)
   ...
   case(2)
      ...
      T_out = max(2.d4/unit_temperature, T_out)
  ...
  subroutine mg_boundary_conditions(iB)
    ...
    case default
      call mpistop( "Set special bound for this Boundary ")
    end select
  end subroutine mg_boundary_conditions


  subroutine floor_velocity(level,qt,qdt,ixI^L,ixO^L,w,x)
    use mod_global_parameters
    ...
    where ((x(ixO^S,1) .lt. 1.2d0) .and. (w(ixO^S,mom(1)) .gt. 1.d-4))
      w(ixO^S,mom(1)) = 1.d-4
    end where
    call hd_to_conserved(ixI^L,ixO^L,w,x)

  end subroutine floor_velocity
  ...
  subroutine set_gravitation_field(ixI^L,ixO^L,wCT,x,gravity_field)
    ...
    gravity_field = zero
    gravity_field(ixO^S,1) = -const_G*mass/radius(ixO^S)**2*(unit_time**2/unit_length)
  ...
  subroutine get_kappa_CAK(ixI^L,ixO^L,w,x,kappa)
    ...
    if(.not. CAK_zero) then
      ...
      call gradient(vel,ixI^L,ixO^L,1,gradv,nghostcells-1)
  ...
  subroutine collapse_to_1D()
    ...
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      ...
      call fld_get_radflux(block%w, block%x, ixG^LL, ixM^LL, radflux, 1)
    ...
    if (mype==0) then
      ...
      do i=1,domain_nx1
        write(unitanalysis,'(8f12.7,2i4)') rr(i), vr(i), vr_sumt(i)/sum_time, vr2(i), vr2_sumt(i)/sum_time,dsqrt( abs(vr2_sumt(i)/sum_time - (vr_sumt(i)/sum_time)**2 )), rhovr_sumt(i)/rho_sumt(i), rho2vr_sumt(i)/rho2_sumt(i) ,lvl_h(i), lvl_l(i)
      enddo
    close(unitanalysis)
    endif
    ...
    if (mype==0) then
      ...
      do i=1,domain_nx1
        write(unitanalysis,'(6f12.7)') rr(i), rho(i), rho_sumt(i)/sum_time, rho2(i), rho2_sumt(i)/sum_time, sum_time*rho2_sumt(i)/rho_sumt(i)**2
      enddo
    close(unitanalysis)
    endif
  ...
  subroutine update_extravars(igrid,level,ixI^L,ixO^L,qt,w,x)
    ...
    double precision :: mass
    double precision :: radius(ixI^S)
    double precision :: g_rad(ixI^S)
    double precision :: big_gamma(ixI^S)
    double precision :: g_grav(ixI^S)
    double precision :: Tgas(ixI^S)
    double precision :: Trad(ixI^S)
    double precision :: kappa(ixO^S)
    double precision :: OPAL(ixO^S)
    double precision :: CAK(ixO^S)
    double precision :: CAK2(ixO^S)
    double precision :: vel(ixI^S)
    double precision :: gradv(ixI^S)
    double precision :: rad_flux(ixO^S,1:ndim)
    double precision :: Lum(ixO^S)
    double precision :: pp_rf(ixO^S)
    double precision :: lambda(ixO^S)
    double precision :: fld_R(ixO^S)
    double precision :: gradOE(ixI^S)
