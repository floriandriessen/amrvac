...
contains
    subroutine mhd_link_eos()
        ...
        if (eos%eos_type == 'FI') then
            if(get_dict_flag(eos%phys_dict, "mhd_hydrodynamic_e")) then
                eos%to_primitive        => mhd_to_primitive_hde
                eos%to_conserved        => mhd_to_conserved_hde
            else if(get_dict_flag(eos%phys_dict, "mhd_semirelativistic")) then
                if(get_dict_flag(eos%phys_dict, "mhd_energy")) then
  ...
  subroutine mhd_to_primitive_semirelati(ixI^L,ixO^L,w,x)
   ...
   {do ix^DB=ixOmin^DB,ixOmax^DB\}
      ...
      if(get_dict_flag(eos%phys_dict, "mhd_internal_e")) then
