...
contains
  ...
  subroutine sample_create_particles()
    ...
    do n=1,num_particles
      ...
      if(ipe_particle == mype) then
        ...
        call sample_update_payload(igrid,x(:,n),v(:,n),q(n),m(n),defpayload,&
           ndefpayload,0.d0)
        particle(n)%payload(1:ndefpayload) = defpayload      
        if (associated(usr_update_payload)) then
          call usr_update_payload(igrid,x(:,n),v(:,n),q(n),m(n),usrpayload,&
             nusrpayload,0.d0)
  ...
  subroutine sample_integrate_particles(end_time)
    ...
    do iipart=1,nparticles_active_on_mype
      ...
      if (associated(usr_particle_position)) call usr_particle_position(x,&
         particle(ipart)%self%index,tloc,tlocnew)
      ...
      ! Update payload
      call sample_update_payload(igrid,x,v,0.d0,0.d0,defpayload,ndefpayload,&
         tlocnew)
      particle(ipart)%payload(1:ndefpayload) = defpayload
      if (associated(usr_update_payload)) then
        call usr_update_payload(igrid,x,v,0.d0,0.d0,usrpayload,nusrpayload,&
           tlocnew)
  ...
  !> Payload update
  subroutine sample_update_payload(igrid,xpart,upart,qpart,mpart,mypayload,&
    ...
    double precision              :: myw(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw),&
       mywold(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw)
    double precision              :: wp, wpold, td
    integer                       :: ii


    ! There are npayload=nw payloads, one for each primitive fluid quantity
    myw(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw) = ps(igrid)%w(ixGlo1:ixGhi1,&
       ixGlo2:ixGhi2,1:nw)
    if (time_advance) mywold(ixGlo1:ixGhi1,ixGlo2:ixGhi2,&
       1:nw) = pso(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw)

    if (saveprim) then
      call phys_to_primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1,ixGlo2,ixGhi1,&
         ixGhi2,myw,ps(igrid)%x)
      if (time_advance) call phys_to_primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,&
         ixGlo1,ixGlo2,ixGhi1,ixGhi2,mywold,ps(igrid)%x)
    end if

    do ii=1,mynpayload
      call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
         ixMhi1,ixMhi2,myw(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ii),ps(igrid)%x,xpart,&
         wp)
      if (time_advance) then
        td = (particle_time - global_time) / dt
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,mywold(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ii),ps(igrid)%x,&
           xpart,wpold)
