...
contains
  ...
  subroutine advect_create_particles()
    ...
    do n=1,num_particles
      ...
      if(ipe_particle == mype) then
        ...
        call advect_update_payload(igrid,x(:,n),v(:,n),q(n),m(n),defpayload,&
           ndefpayload,0.d0)
  ...
  subroutine advect_integrate_particles(end_time)
    ...
    do iipart=1,nparticles_active_on_mype
      ...
      case (ARK4)
        ...
        h1 = dt_p/2.0d0
        call odeint(x,ndir,tloc,tlocnew,eps,h1,hmin,nok,nbad,derivs_advect,&
           rkqs,ierror)
      ...
      ! Update payload
      call advect_update_payload(igrid,x,v,0.d0,0.d0,defpayload,ndefpayload,&
         tlocnew)
  ...
  !> Payload update
  subroutine advect_update_payload(igrid,xpart,upart,qpart,mpart,mypayload,&
    ...
    if (mynpayload > 0 ) then
      if (.not.time_advance) then
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,iw_rho),&
           ps(igrid)%x,xpart,rho)
      else
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,pso(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,iw_rho),&
           ps(igrid)%x,xpart,rho1)
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,iw_rho),&
           ps(igrid)%x,xpart,rho2)
  ...
  function advect_get_particle_dt(partp, end_time) result(dt_p)
    ...
    do idims=1,ndim
      dtdims(idims)=minval(ps(partp%igrid)%ds(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
         idims))/(max(v(idims),smalldouble))
    end do
