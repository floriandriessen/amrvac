...
contains
  ...
  subroutine getlQgrid(lQgrid,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
     ixOmax1,ixOmax2,qt,trelax,x,mode,vtim,vlim)
    use mod_global_parameters
    integer, intent(in)             :: ixImin1,ixImin2,ixImax1,ixImax2,&
        ixOmin1,ixOmin2,ixOmax1,ixOmax2, mode, vtim, vlim
    double precision, intent(in)    :: qt, trelax, x(ixImin1:ixImax1,&
       ixImin2:ixImax2,1:ndim)
    double precision, intent(inout) :: lQgrid(ixImin1:ixImax1,ixImin2:ixImax2)
    integer                         :: Bp,Bt1,Bt2,ix1,ix2,i
            ...
            do ix1 = ixOmin1,ixOmax1
              ...
              if(x(ix1,ix2,1) .lt. 0.d0) then
                lQgrid(ix1,ix2) = tl1 * (va1(Bp + (i-4)*vlim)*(one-va0)+va0) + &
                   tl2 * (va1(Bp + (i-3)*vlim)*(one-va0)+va0) + tl3 * (va1(Bp &
                   + (i-2)*vlim)*(one-va0)+va0) + tl4 * (va1(Bp + &
                   (i-1)*vlim)*(one-va0)+va0) + tl5 * (va1(Bp + &
                   (i)*vlim)*(one-va0)+va0) + tl6 * (va1(Bp + &
                   (i+1)*vlim)*(one-va0)+va0)
              endif
            enddo
            enddo
            exit
            ...
            do ix1 = ixOmin1,ixOmax1
              ...
              if(x(ix1,ix2,1) .ge. 0.d0) then
                lQgrid(ix1,ix2) = tr1 * (va2(Bp + (i-4)*vlim)*(one-va0)+va0) + &
                   tr2 * (va2(Bp + (i-3)*vlim)*(one-va0)+va0) + tr3 * (va2(Bp &
                   + (i-2)*vlim)*(one-va0)+va0) + tr4 * (va2(Bp + &
                   (i-1)*vlim)*(one-va0)+va0) + tr5 * (va2(Bp + &
                   (i)*vlim)*(one-va0)+va0) + tr6 * (va2(Bp + &
                   (i+1)*vlim)*(one-va0)+va0)
              endif
            enddo
            enddo
    ...
    case(2)
      ...
      do ix1 = ixOmin1,ixOmax1
        do ix2 = ixOmin2,ixOmax2
          Bp = floor((x(ix1,ix2,1) - xprobmin1) / stt) + 1
          if(x(ix1,ix2,1) .lt. 0.d0) then
            lQgrid(ix1,ix2) = tr1 * (va1(Bp + Bt1*vlim)*(one-va0)+va0) + tr3 * &
               (va1(Bp+(Bt1+1)*vlim)*(one-va0)+va0)
          else
            lQgrid(ix1,ix2) = tr2 * (va2(Bp + Bt2*vlim)*(one-va0)+va0) + tr4 * &
  ...
  subroutine randomV(va,vtim,vlim,mnx,num,si,filename)
    ...
    do kk = 1, vtim
        ...
        do i = 1, num
          ...
          do j = 1, mnx  
            vn(i,j) = c*sin(2.*dpi*(j*1.d0/(mnx-1)*lth)/lambda+&
               2.*dpi*2.*(ll-0.5))
          end do
        end do     
        rms = 0.d0
