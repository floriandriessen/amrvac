...
contains
  ...
  subroutine add_split_source(prior)
    ...
    double precision :: w1(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1:nw)
    double precision :: qt
      ...
      case (sourcesplit_sfs)
        ...
        do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
           ...
           call phys_to_primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1,ixGlo2,&
              ixGhi1,ixGhi2,w1,ps(igrid)%x)
           call addsource2(0.5d0*dt,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
              ixMhi1,ixMhi2,1,nw,qt,ps(igrid)%w,w1,qt,ps(igrid)%w,ps(igrid)%x,&
              .true.,src_active)
        end do
        !$OMP END PARALLEL DO
      case (sourcesplit_sf)
        ...
        do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
           ...
           call phys_to_primitive(ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1,ixGlo2,&
              ixGhi1,ixGhi2,w1,ps(igrid)%x)
           call addsource2(dt  ,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
              ixMhi1,ixMhi2,1,nw,qt,ps(igrid)%w,w1,qt,ps(igrid)%w,ps(igrid)%x,&
              .true.,src_active)
        end do
        !$OMP END PARALLEL DO
      case (sourcesplit_ssf)
        ...
        do iigrid=1,igridstail_active; igrid=igrids_active(iigrid);
           ...
           call addsource2(0.5d0*dt,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixGlo1,ixGlo2,&
              ixGhi1,ixGhi2,1,nw,qt,ps(igrid)%w,w1,qt,ps(igrid)%w,ps(igrid)%x,&
              .true.,src_active)
           call addsource2(dt  ,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
              ixMhi1,ixMhi2,1,nw,qt,ps(igrid)%w,w1,qt,ps(igrid)%w,ps(igrid)%x,&
              .true.,src_active)
  ...
  subroutine addsource2(qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
     ixOmax1,ixOmax2,iwmin,iwmax,qtC,wCT,wCTprim,qt,w,x,qsourcesplit,&
     src_active)
    use mod_global_parameters
    ...
    call phys_add_source(qdt,ixImin1,ixImin2,ixImax1,ixImax2,ixOmin1,ixOmin2,&
       ixOmax1,ixOmax2,wCT,wCTprim,w,x,qsourcesplit,tmp_active)
