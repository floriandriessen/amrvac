...
contains
  ...
  subroutine Lorentz_create_particles()
    ...
    do n = 1, num_particles
      ...
      if (ipe_particle == mype) then
        ...
        call Lorentz_update_payload(igrid,x(:,n),v(:,n)*lfac,q(n),m(n),&
           defpayload,ndefpayload,0.d0)
        particle(n)%payload(1:ndefpayload) = defpayload
        if (associated(usr_update_payload)) then
          call usr_update_payload(igrid,x(:,n),v(:,n)*lfac,q(n),m(n),&
             usrpayload,nusrpayload,0.d0)
  ...
  subroutine Lorentz_integrate_particles(end_time)
    ...
    double precision                  :: xp(ndir), xpm(ndir), xpc(ndir),&
        xpcm(ndir)
    ...
    do iipart=1,nparticles_active_on_mype
      ...
      case (cylindrical)
        e(r_) = -vfluid(phi_)*b(z_)+vfluid(z_)*b(phi_) + &
           particles_eta*current(r_)
        e(phi_) = vfluid(r_)*b(z_)-vfluid(z_)*b(r_) + &
           particles_eta*current(phi_)
        e(z_) = -vfluid(r_)*b(phi_)+vfluid(phi_)*b(r_) + &
           particles_eta*current(z_)
      end select
      if (particles_etah > zero) then
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1),&
           ps(igrid)%x,xpm,rho)
        if (time_advance) then
          td = (tp+dt_p/2.d0 - global_time) / dt
          call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
             ixMhi1,ixMhi2,pso(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1),&
             ps(igrid)%x,xpm,rhoold)
          rho = rhoold * (1.d0-td) + rho * td
        end if
        select case (coordinate)
        case (Cartesian,Cartesian_stretched,spherical)
          e(1) = e(1) + particles_etah/rho * (current(2)*b(3) - &
             current(3)*b(2))
          e(2) = e(2) + particles_etah/rho * (-current(1)*b(3) + &
             current(3)*b(1))
          e(3) = e(3) + particles_etah/rho * (current(1)*b(2) - &
             current(2)*b(1))
        case (cylindrical)
          e(r_) = e(r_) + particles_etah/rho * (current(phi_)*b(z_) - &
             current(z_)*b(phi_))
          e(phi_) = e(phi_) + particles_etah/rho * (-current(r_)*b(z_) + &
             current(z_)*b(r_))
          e(z_) = e(z_) + particles_etah/rho * (current(r_)*b(phi_) - &
             current(phi_)*b(r_))
        end select
      end if
...
!      end if
    ...
    case(LM)
      ...
      do while(abserr > tol .and. nk < nkmax)
        ...
        J21 = - q*dtp/m * (- C1(1) * b(3) + C2(1) * (upartk(3) + upart(3)) * &
           b(1))
        J22 = 1. - q*dtp/m * (- C2(2) * (upartk(1) + upart(1)) * b(3) + C2(2) &
           * (upartk(3) + upart(3)) * b(1))
  ...
  subroutine Lorentz_update_payload(igrid,xpart,upart,qpart,mpart,mypayload,&
     mynpayload,particle_time)
    use mod_global_parameters
    use mod_geometry
    integer, intent(in)           :: igrid,mynpayload
    double precision, intent(in)  :: xpart(1:ndir),upart(1:ndir),qpart,mpart,&
       particle_time
    double precision, intent(out) :: mypayload(mynpayload)
    double precision              :: b(3), e(3), tmp(3), lfac, vfluid(3),&
        current(3), rho, rhoold, td
    ...
    if (particles_etah > zero) then
      call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
         ixMhi1,ixMhi2,ps(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1),ps(igrid)%x,&
         xpart,rho)
      if (time_advance) then
        td = (particle_time - global_time) / dt
        call interpolate_var(igrid,ixGlo1,ixGlo2,ixGhi1,ixGhi2,ixMlo1,ixMlo2,&
           ixMhi1,ixMhi2,pso(igrid)%w(ixGlo1:ixGhi1,ixGlo2:ixGhi2,1),&
           ps(igrid)%x,xpart,rhoold)
        rho = rhoold * (1.d0-td) + rho * td
      end if
      select case (coordinate)
      case (Cartesian,Cartesian_stretched,spherical)
