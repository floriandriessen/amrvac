...
contains
  ...
  subroutine getbc(time,qdt,psb,nwstart,nwbc,req_diag)
    ...
    contains
          ...
          if(ipole==0) then
            ...
            if(stagger_grid) then
              do idir=1,ndim
                ...
                ixRmax1=ixR_srl_stg_max1(idir,n_i1)
                ixRmax2=ixR_srl_stg_max2(idir,n_i2);
                call pole_copy_stg(psb(ineighbor)%ws,ixRmin1,ixRmin2,ixRmax1,&
                   ixRmax2,psb(igrid)%ws,ixSmin1,ixSmin2,ixSmax1,ixSmax2,idir,&
                   ipole)
              end do
            end if
          end if
          ...
          if(ipole==0) then
            ...
            if(stagger_grid) then
              do idir=1,ndim
                ...
                !! Fill ghost cells
                call pole_copy_stg(psb(ineighbor)%ws,ixRmin1,ixRmin2,ixRmax1,&
                   ixRmax2,psc(igrid)%ws,ixSmin1,ixSmin2,ixSmax1,ixSmax2,idir,&
                   ipole)
              end do
            end if
          end if
          ...
          if (ipole==0) then    
            ...
            do idir=1,ndim
             ...
             pole_buf%ws(ixSmin1:ixSmax1,ixSmin2:ixSmax2,&
                ...
                idir)))
             ibuf_recv_srl=ibuf_next
             call pole_copy_stg(psb(igrid)%ws,ixRmin1,ixRmin2,ixRmax1,ixRmax2,&
                pole_buf%ws,ixSmin1,ixSmin2,ixSmax1,ixSmax2,idir,ipole)
            end do
          end if
        end if
              ...
              case (2)
               ...
               do idir=1,ndim
                 ...
                 pole_buf%ws(ixRmin1:ixRmax1,ixRmin2:ixRmax2,&
                    ...
                    shape=shape(psb(igrid)%ws(ixRmin1:ixRmax1,ixRmin2:ixRmax2,&
                    idir)))
                 call pole_copy_stg(psb(igrid)%ws,ixRmin1,ixRmin2,ixRmax1,&
                    ixRmax2,pole_buf%ws,ixRmin1,ixRmin2,ixRmax1,ixRmax2,idir,&
                    ipole)
                 ibuf_recv_r=ibuf_next
               end do
             end if
             ...
             if(ipe_neighbor==mype) then
               ...
               if(stagger_grid) then
                 do idir=1,ndim
                   ...
                   ixRmax1=ixR_p_stg_max1(idir,n_inc1)
                   ixRmax2=ixR_p_stg_max2(idir,n_inc2);
                   call pole_copy_stg(psc(ineighbor)%ws,ixRmin1,ixRmin2,&
                      ixRmax1,ixRmax2,psb(igrid)%ws,ixSmin1,ixSmin2,ixSmax1,&
                      ixSmax2,idir,ipole)
                 end do
               end if
             end if
          ...
          if (ipole==0) then   !! There is no pole 
            ...
            do idir=1,ndim
              ...
              pole_buf%ws(ixRmin1:ixRmax1,ixRmin2:ixRmax2,&
                 ...
                 shape=shape(psc(igrid)%ws(ixRmin1:ixRmax1,ixRmin2:ixRmax2,&
                 idir)))
              call pole_copy_stg(psc(igrid)%ws,ixRmin1,ixRmin2,ixRmax1,ixRmax2,&
                 pole_buf%ws,ixRmin1,ixRmin2,ixRmax1,ixRmax2,idir,ipole)
              ibuf_recv_p=ibuf_next
            end do
          end if
        end if
      ...
      subroutine pole_copy_stg(wrecv,ixRmin1,ixRmin2,ixRmax1,ixRmax2,wsend,&
         ixSmin1,ixSmin2,ixSmax1,ixSmax2,idirs,ipole)
      
        integer, intent(in) :: ixRmin1,ixRmin2,ixRmax1,ixRmax2,ixSmin1,ixSmin2,&
           ixSmax1,ixSmax2,idirs,ipole

        double precision :: wrecv(ixGslo1:ixGshi1,ixGslo2:ixGshi2,1:nws),&
            wsend(ixGslo1:ixGshi1,ixGslo2:ixGshi2,1:nws)
        integer :: iB, iside
